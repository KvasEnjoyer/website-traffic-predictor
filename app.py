...
with tab_eda:
    ...
    st.markdown(
        "Порівняння трафіку та довгострокових трендів для усіх вебсайтів.")
...
with tab_diagnostics:
    ...
    st.markdown(
        "Оцінка припущень класичної лінійної регресійної моделі для МНК залишків $\\epsilon_t$ за допомогою формальних статистичних тестів.")
    ...
    with col_norm_left:
        ...
        if jb_p < 0.05:
            ...
            st.info(
                "Залишки не мають суто нормального розподілу. Стандартні помилки та довірчі інтервали можуть відхилятися від теоретичних значений.")
        else:
    ...
    with col_hom_left:
        ...
        if lev_p < 0.05:
            ...
            st.success(r"$H_0 \text{ не відхиляється} \text{ на рівні значущості } \alpha = 0.05$")
            st.info(
                "Дисперсія залишків залишається стабільною. Оцінки стандартних помилок МНК є надійними.")
